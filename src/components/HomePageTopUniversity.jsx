...
export default function HomePageTopUniversity({ unis, countrySlug }) {
  // If no universities are found for this country
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-12">
          ...
          <p className="text-slate-500 font-medium capitalize">
            Recognized institutions for international students in {countrySlug}.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {unis.map((uni) => (
            <Link
              href={`/universities/${uni.country}/${uni.name.toLowerCase().replace(/ /g, "-")}`}
              key={uni.id}
